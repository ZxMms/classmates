...
class Table_baseconfig extends Table
{
    ...
    static public function edit_minxinswitch($minxinswitch){
        ...
        return $mypdo_minxin->sqlupdate($mypdo_minxin->prefix. 'baseconfig',$param,$where);
    }

    static public function getStatus(){
        global $mypdo;

        $sql = "select * from " .$mypdo->prefix. "baseconfig where baseconfig_key= 'minxin_switch'";
//              print_r($sql);exit();
        $r = $mypdo->sqlQuery($sql);
        if($r){
            //     print_r($r);exit();
            return $r[0]['baseconfig_value'];
        }else{
            return 0;
        }
    }
    static public function updateCountInfo($key){
        ...
        $param = array(
            'baseconfig_value'    => array('expression','baseconfig_value+1')
        );
        $where = array('baseconfig_key'=>array('string',$key));
//echo $param;
        return $mypdo->sqlupdate($mypdo->prefix.'baseconfig', $param, $where);
    ...
    static public function getCountNum(){
        ...
        $sql = "select * from ".$mypdo->prefix."baseconfig where baseconfig_key = 'minxin_count'";
//echo $sql;
        $rs = $mypdo->sqlQuery($sql);
        $r  = array();
        if($rs){
    ...
    static public function getRongChengSlogn_switch(){
        ...
        $sql = "select * from " .$mypdo->prefix. "baseconfig where baseconfig_key= 'minxin_content_manage'";
//              print_r($sql);exit();
        $r = $mypdo->sqlQuery($sql);
        if($r){
            //     print_r($r);exit();
